...
void ResetArrays(int *mutex, float *x, float *y, float *z, float *mass, int *count, int *root, int *sorted, int *child, int *index, float *left, float *right, float *bottom, float *top, float* front, float *back,int p_count, int node_count)
{
	aux_kernel_initialize_device_arrays<<<params.gridSize, params.blockSize>>>(mutex, x, y, z, mass, count, root, sorted, child, index, left, right, bottom, top, front, back, p_count, node_count);
}